...
This module is the aggregate module of AnvilLib, representing the foundation of the entire library. It does not contain
functional code itself but serves as the aggregate entry point for all submodules, providing unified version management,
build publishing, and dependency coordination.

## Project Structure

AnvilLib uses a multi-module Gradle project organization. The root project `anvillib` uniformly configures build
information for all submodules via `build.gradle`.
