...
The package `dev.anvilcraft.lib.v2.piston` solves the issue of vanilla pistons being unable to push blocks with block
entities. Through a set of Mixins and interfaces, it allows blocks to carry custom
...
In vanilla, a piston pushing a block with a block entity has no effect. This module injects into the piston movement
logic:
...
- `IMoveableEntityBlock` extends `EntityBlock`; if your block is already a `BaseEntityBlock` subclass, you only need to
  additionally implement this interface
