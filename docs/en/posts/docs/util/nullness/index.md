...
Located at `dev.anvilcraft.lib.v2.util.nullness`, providing non-null/nullable type annotations and corresponding
...
All interfaces support composition (`andThen`) and a static `noop()` factory (no-op). `NonNullSupplier` additionally
provides `lazy()` and `of(Supplier)` conversion.

## Import Paths Across Versions

Since `module.util` was not released as a standalone module in some versions, the import path for the `nullness` package
varies by version:
