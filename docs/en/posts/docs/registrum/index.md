...
The package `dev.anvilcraft.lib.v2.registrum` provides a declarative registration system based on fluent builders,
greatly simplifying the registration and data generation workflow for blocks, items, entities, block entities, menus,
fluids, and more in Minecraft mods.

::: info
Portions of this module are based on [Registrate](https://github.com/tterrag1098/Registrate), under the Mozilla Public
License 2.0.
:::
...
`object("name")` sets a **persistent state**; subsequent builder calls will reuse that name until the next `object()`
call.
...
- Registration operations should be completed during the mod construction phase (in the `@Mod` constructor or before
  `FMLCommonSetupEvent`)
...
- Thread safety: `AbstractRegistrum` is not thread-safe; all registration operations should be performed on the main
  thread
- Entry names are globally unique (within the modId namespace); registering a duplicate entry with the same name will
